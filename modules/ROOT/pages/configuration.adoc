...
val dfProduct = spark.read.format("org.neo4j.spark.DataSource")
        ...
        .load()
----
== Custom authentication supplier label:new[v.5.3.9]
Starting from version 5.3.9 Neo4j Spark Connectors provides an option to use custom authentication supplier in addition to
already supported authentication types, such as `none`, `basic`, `kerberos`, `custom` and `bearer`.

Authentication supplier needs to implement interface `org.neo4j.connectors.authn.AuthenticationTokenSupplierFactory` from `org.neo4j.connectors:commons-authn-spi` library.
...
An `AuthenticationToken` instances is an abstract representation of an arbitrary authentication to be presented to the Neo4j server and can contain a principal and its credentials (username and password), a token or other ways of confirming an identity.
...
Custom authentication supplier can be used via the service loader mechanism and setting configuration parameter `authentication.type` to the name
of the supplier. Additional parameters can be as configuration options prefixed with `authentication.$name.`.

=== Keycloak authentication supplier example

Keycloak authentication supplier in implemented in the `org.neo4j.connectors:commons-authn-keycloak` library.
It consists of a factory class creating an instance of an authentication token supplier;
[source, java]
----
include::example$java/KeycloakOIDCAuthenticationSupplierFactory.java[]
----

and a supplier that collects an access token from a Keycloak server and refreshes it on expiration.
...
Configuration options for it would look like
[source, scala]
----
val df = spark.read
...
* any xref:_custom_authentication_supplier_v_5_3_9[custom authentication supplier] label:new[v.5.3.9]
