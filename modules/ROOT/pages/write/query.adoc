...
The `script` options allow running one or more Cypher queries ("scripts") _before_ executing the write operation.
The result of the last query can be used in a subsequent query, for example to inject query parameters.

Use the `script` option to run a *single* Cypher query.
To run *multiple* queries, use the indexed `script.N` options, where each query is passed in its own option. label:new[New in v5.5]
...
df.write
  ...
  .option("script.3", "RETURN 36 AS age")
  .save()
----
...
`scriptResult` is the result of the last script, which in this case is `RETURN 36 AS age`.
