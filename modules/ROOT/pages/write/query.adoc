...
Script options allow running one or more Cypher queries before executing the write operation.
...
To run *multiple* queries, use the indexed `script.N` options label:new[v5.5], where each query is passed in its own option.
...
`scriptResult` is the result of the last script query, which in this case is `RETURN 36 AS age`.
