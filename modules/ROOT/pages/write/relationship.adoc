...
The `mode` defines whether the query uses a `CREATE` or a `MERGE` clause (see xref:writing.adoc#save-mode[save mode]).
...
.Example of `native` strategy, `Append` mode for relationship, `Append` mode for source/target nodes
...
It does not require any specific schema for the DataFrame as you can specify which columns to write as node and relationship properties.

[#rel-specify-keys]
.Example of `keys` strategy, `Append` mode for relationship, `Append` mode for source/target nodes
[source, scala, role=nocollapse]
----
...
The examples from the previous section use the `Append` mode for both relationships and nodes; this means that, every time the code is run, new relationships and new nodes will be created.

Node save strategies different from `Append` can alter this behaviour.
...
----
UNWIND $events AS event
MATCH (source:Person {id: event.source.keys.id})
...
----
UNWIND $events AS event
MERGE (source:Person {id: event.source.keys.id})
SET source += event.source.properties
