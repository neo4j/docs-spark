...
. Select *Search Packages* and search for either `neo4j-spark-connector` (Spark Packages) or `neo4j-connector-apache-spark` (Maven Central) from the `neo4j` organization, then *Select* the most recent one.
+
[NOTE]
====
