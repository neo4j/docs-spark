...
* Read <<read-node, nodes>> using the `labels` option.
* Read a <<read-rel, relationship>> with source and target nodes using the `relationship` option.
...
If you need more flexibility, you can also run a xref:reading-cypher.adoc[custom Cypher query] using the `query` option.

[[read-node]]
== Node

You can read nodes by specifying one or more labels in the `labels` option:
