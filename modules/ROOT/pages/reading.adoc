...
The standard way to read data from Neo4j with the Spark connector is via data source options.
...
There are two main approaches to read data from a Neo4j database via the connector:

* The <<read-node, node>> approach using the `labels` option
* The <<read-rel, relationship>> approach using the `relationship` option

A third option is to use a xref:reading-cypher.adoc[custom Cypher query].

[[read-node]]
== Node

You can read single nodes by specifying one or more labels in the `labels` option:
...
[NOTE]
====
A list of multiple labels can be specified either with or without a starting colon.
For example, `Person:Customer` and `:Person:Customer` are considered the same.
====
...
You can read nodes connected by a relationship by specifying the relationship type, the source node labels, and the target node labels.
...
Additional columns are added depending on the value of `relationship.nodes.map` option.

|===
|`relationship.nodes.map` set to `false` (default)|`relationship.nodes.map` set to `true`

a|
* `<sourceId>`: internal Neo4j ID of source node
* `<sourceLabels>`: list of labels for source node
* `<targetId>`: internal Neo4j ID of target node
* `<targetLabels>`: list of labels for target node
