...
* <<save-mode, Save mode>>: defines whether the query uses a `CREATE` or a `MERGE` clause.
...
* `relationship.source.labels` and `relationship.target.labels` define the labels to assign to source and target nodes.
* `relationship.source.save.mode`: <<node-save-modes>>
+
...
When using `MATCH` or `MERGE` for `relationship.source.save.mode`, you need to specify keys that identify the nodes.
