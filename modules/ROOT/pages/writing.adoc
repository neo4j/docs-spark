...
|`schema.type.constraint` |true/false |false | Creates the type constraints for nodes/relationships enforcing the type from the DataFrame schema
|===
...
In order to leverage this feature we added the option `schema.type.constraint` that will use the DataFrame schema in order to enforce the type.
