...
[NOTE]
since the version **5.3.0** option **schema.optimization.type** is deprecated in favor of **`schema.optimization.node.\*`**, **`schema.optimization.relationship.*`** and **schema.type.constraint**.
...
|`schema.optimization.node.unique` |true/false |false | Create the `UNIQUE` constraint for the properties defined in the option `node.keys`
|`schema.optimization.node.key` |true/false |false | Create the `NODE KEY` constraint for the properties defined in the option `node.keys`
|`schema.optimization.relationship.unique` |true/false |false | Create the `UNIQUE` constraint for the properties defined in the option `relationship.keys`
|`schema.optimization.relationship.key` |true/false |false | Create the `RELATIONSHIP KEY` constraint for the properties defined in the option `relationship.keys`
|`schema.type.constraint` |true/false |false | Creates the type constraints for node/relationships enforcing the type from the DataFrame schema
|===

===== Node constraints

====== Using `schema.optimization.node.unique`
...
====== Using `schema.optimization.node.key`
...
====== Using `schema.optimization.relationship.unique`
...
----
    ds
      ...
      .option("relationship.target.save.mode", "Overwrite")
      .option("schema.optimization.relationship.unique", true)
...
====== Using `schema.optimization.relationship.key`
...
----
    ds
      ...
      .option("relationship.target.node.keys", "idTarget:id")
      .option("relationship.target.save.mode", "Overwrite")
      .option("schema.optimization.relationship.key", true)
      .option("relationship.keys", "foo,bar")
      .save()
----
...
Since Neo4j 5.11 the database allow to create type constraints for node and relationship properties.
In oder to leverage this feature we added the option `schema.type.constraint` that will use the DataFrame schema in order to enforce the type.
...
----
    ds.write
      ...
      .option("labels", ":Person:Customer")
      .option("node.keys", "surname")
      .option("schema.type.constraint", true)
      .save()
----
